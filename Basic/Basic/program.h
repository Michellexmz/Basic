...
struct Line {
   ...
   Line *next;
   string linestr;
   Line(int x, string str):linenum(x),linestr(str),stmt(NULL),next(NULL){};
};
 ...
 */
   ...
   void setParsedStatement(int lineNumber, Statement *statement);
 ...
 */
   ...
   bool findLineNumber(int lineNumber);
   void run(EvalState & state);
   void list();
   void clearOrder(Program& program,EvalState & state);
   void help();
